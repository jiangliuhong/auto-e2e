...
4. Create `.auto-e2e/task-spec.json` from the requested behavior. Make every acceptance criterion atomic, observable in the product, and independent of implementation details.
5. Run `auto-e2e --project-root <workspace> doctor --json`, then `auto-e2e --project-root <workspace> run --json`.
6. Treat the run as complete only when its status is `passed` and every criterion passed. For `failed` or `blocked`, report the failed criterion, actual observation, and proof or actionable blocker. Do not claim success merely because the command launched.
